...
% Use nrbevalIGES for evaluation.
%
% Usage:
...
elseif length(nurbs.order)==1
    ...
    if nurbs.order>1
        % Do the derivation
        dnurbs.coefs=(nurbs.order-1)*(nurbs.coefs(:,2:nurbs.number,:)-nurbs.coefs(:,1:(nurbs.number-1),:))./repmat((nurbs.knots((1+nurbs.order):(nurbs.number+nurbs.order-1))-nurbs.knots(2:nurbs.number)),[4,1]);
